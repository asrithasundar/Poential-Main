...
class D1(ABC):
    ...
    def force_num(self, x: ArrayLike, h=0.00001):
        """
        Calculate the force F(x) numerically via the central finite difference.
        Since the potential is one-dimensional, the force is vector with one element.
    ...
    # Hessian matrix, numerical expression via second order finite difference
